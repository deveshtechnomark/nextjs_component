...
interface TableProps {
  ...
  expandable?: boolean;
  JsxComponents?: Array<React.ComponentType<any> | JSX.Element | React.ReactNode>;
  actionDesc?: any[];
...
const DataTable: React.FC<TableProps> = (props) => {
  ...
  const handleSort = (column: string, isSortable: boolean) => {
    ...
    // Restructure data to organize column-wise
    const columnWiseData = {};
    sortedData.forEach((row) => {
      for (const key in row) {
        if (!columnWiseData[key]) {
          columnWiseData[key] = [row[key]];
        } else {
    ...
    // Update the state for each field separately
    for (const field in columnWiseData) {
      setFilteredData((prevState) => ({
        ...prevState,
        [field]: columnWiseData[field],
      }));
    }
  ...
  return (
    ...
    >
      <table className="w-full">
        ...
        <tbody>
          {[...Array(Math.max(...filteredData.map(item => Object.keys(item).length)))].map((_, index) => (
            <React.Fragment key={index}>
              ...
              >
                {props.expandable && (
                  <td className="sm:w-[56px]">
                    ...
                    {filteredData[index] && filteredData[index][nestedKey] && filteredData[index][nestedKey].length > 0 ? (
                      <button
                ...
                {props.headers.map((header, index) => (
                  ...
                  >
                    <span className="flex justify-start items-center">
                      {typeof filteredData[index] !== "undefined" &&
                        typeof filteredData[index][header.field] === "string" &&
                        filteredData[index][header.field].startsWith("http") ? (
                        <img
                          src={filteredData[index][header.field]}
                          alt="Item"
                        ...
                        <div>
                          {React.createElement(
                            props.JsxComponents[header.field],
                            {
                              data: filteredData[index][header.field],
                            }
                          )}
                        </div>
                      ) : (
                        filteredData[index] && filteredData[index][header.field]
                      )}
                    </span>
                  </td>
                ))}
                ...
                {props.action &&
                  ...
                  props.actions.map((action) => (
                    <td
                      ...
                      key={action}
                      className={`cursor-pointer text-[16px] sm:text-[14px] font-bold uppercase ${props.actionHeading && "w-[10px] pr-5"} ${props.actionSticky ? `${Style.sticky_action_column}` : ""
                        } ${props.sticky && index === selectedRowIndex ? "right-0 !z[2]" : ""}`}
                    >
                      <div onClick={() => {
                        if (props.getRowId) {
                          props.getRowId(filteredData[index]);
              ...
              {props.expandable && filteredData.map((item, rowIndex) =>
                expandedRows[rowIndex] && nestedKey && item[nestedKey] ? (<>
